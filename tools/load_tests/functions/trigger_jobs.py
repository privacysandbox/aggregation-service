...
def generate_payload(debug_run, attribution_report_to, input_data_blob_prefix,
                     input_data_bucket_name, output_data_blob_prefix,
                     output_data_bucket_name, output_domain_bucket_name,
                     output_domain_blob_prefix):
  job_request_id = str(uuid.uuid4())
  output_prefix = (
      "output-data/{job_request_id}/{output_data_blob_prefix}".format(
          job_request_id=job_request_id,
          output_data_blob_prefix=output_data_blob_prefix))
  return {
      "job_request_id": job_request_id,
...
def lambda_handler(event, context):
  ...
  for _ in range(num_requests):
    payload_dict = generate_payload(
        debug_run, attribution_report_to, input_data_blob_prefix,
        input_data_bucket_name, output_data_blob_prefix,
        output_data_bucket_name, output_domain_bucket_name,
        output_domain_blob_prefix)

    payload = json.dumps(payload_dict)
    headers = create_headers(payload, access_key, secret_key, host, region,
                             service)
    print(url)
    print(headers)
    print(payload)
