...
def validate_event(event):
  ...
  keys = [
      "access_key", "base_url", "host", "job_request_ids", "region",
      "secret_key", "service"
  ]
  for key in keys:
    if key not in event:
...
def create_headers(access_key, secret_key, host, region, service,
                   job_request_id):
  t = datetime.utcnow()
...
def extract_from_response(data):
  ...
  error_summary = data.get("result_info", {}).get("error_summary",
                                                  {"error_counts": []})
  error_count = 0
...
def make_get_request(job_request_id, event):
  ...
  headers = create_headers(event["access_key"], event["secret_key"],
                           event["host"], event["region"], event["service"],
                           job_request_id)
...
def check_jobs_status(event, failed_job_ids, received_at, finished_at):
  ...
  for job_request_id in job_request_ids:
    overall_status = (
        check_job_status(job_request_id, event, received_at, finished_at))
...
def lambda_handler(events, context):
  ...
  for event in events:
    if not validate_event(event):
      raise Exception(
          "Please provide access_key, secret_key, host and base_url")
