...
/**
 * Validates that the Attribution Reporting Debug API type report's SharedInfo fields used in
 * Privacy Budget Key generation are present and not null/empty.
 */
public class AttributionReportingDebugPrivacyBudgetKeyFieldsValidator
    implements PrivacyBudgetKeyValidator {

  @Override
  public Optional<ErrorMessage> validatePrivacyBudgetKey(SharedInfo sharedInfo) {
    if (isFieldNonEmpty(sharedInfo.reportingOrigin())
        && isFieldNonEmpty(sharedInfo.destination())
